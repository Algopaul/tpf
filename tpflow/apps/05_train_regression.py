...
from tpflow.model import (get_regression_model, regression_rollout,
...
def batch_prep(batch):
  ...
  return batch_dict['data'], batch_dict['next'], batch_dict['time'], batch_dict['param']


@hydra.main(
    version_base=None, config_name='regression', config_path='../../conf')
@log_duration()
def main(cfg: RegressionTraining) -> None:
  ...
  with wandb.init(
    ...
    logging.info('Data prepared: %d train batches, %d val batches',
...
def _log_rollout_video(model, cfg: RegressionTraining, run, step: int):
  ...
  traj_param = (np.array(traj_file['param'])[:cfg.n_rollout]
                if 'param' in traj_file else np.ones(cfg.n_rollout))
