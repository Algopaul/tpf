...
def convert_split(
    name: str, split: str, fields: tuple, block_size: int, blocks_per_shard: int
):
    ...
    logging.info(
        "%s: %d samples → %d blocks of %d → shards of %d blocks",
        split,
        n,
        n_blocks,
        block_size,
...
@hydra.main(version_base=None, config_name="wds_convert", config_path="../../conf")
@log_duration()
